...
function renderResults(data) {
    ...
    // ── Auto-fill monthly tab city ─────────────────────────────────────────
    const maCityEl = document.getElementById("ma-city");
    if (maCityEl && !maCityEl.value) maCityEl.value = loc.name;

    // ── Render 7-day weather outlook ───────────────────────────────────────
    if (typeof renderWeeklyOutlook === "function" && data.daily_outlook) {
        const weeklySection = document.getElementById("weekly-section");
        if (weeklySection) weeklySection.style.display = "";
        renderWeeklyOutlook(data.daily_outlook);
    }

    // ── Render battery storage guide ───────────────────────────────────────
    if (typeof renderBatteryGuide === "function" && data.forecast) {
        const batSection = document.getElementById("battery-section");
        if (batSection) batSection.style.display = "";
        renderBatteryGuide(data.forecast);
    }
}
